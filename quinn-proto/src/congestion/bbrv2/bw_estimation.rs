...
pub(crate) struct BandwidthEstimation {
    ...
    // BBRv2 specific fields
    short_term_max: MinMax,
    long_term_max: MinMax,
}
...
impl BandwidthEstimation {
    ...
    pub(crate) fn on_ack(
        ...
        if !app_limited && self.max_filter.get() < bandwidth {
            ...
            // Update BBRv2 specific filters
            self.short_term_max.update_max(round, bandwidth);
            self.long_term_max.update_max(round / 10, bandwidth); // Update less frequently
        }
    }
    ...
    #[allow(dead_code)]
    pub(crate) fn get_short_term_estimate(&self) -> u64 {
        self.short_term_max.get()
    }
    
    #[allow(dead_code)]
    pub(crate) fn get_long_term_estimate(&self) -> u64 {
        self.long_term_max.get()
    }
